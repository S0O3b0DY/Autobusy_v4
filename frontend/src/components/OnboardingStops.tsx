...
// components
import { X, Search, PinAlt, InfoCircle, Trash, Eraser } from "@boxicons/react"
...
export default function OnboardingStops() {
  ...
  const filteredStops = useMemo(() => {
    ...
    return busStops.map(stop => {
      ...
      let score = 0
        ...
        return { stop, score }
    }).filter(Boolean).sort((a: any, b: any) => b.score - a.score).map((x: any) => x.stop).slice(0, 10)
  ...
  async function addToFavorites(stop: BusStopData | undefined) {
    ...
    if (favoriteStops.includes(stop.id)) {
      ...
      await setDoc(userRef, { favoriteStops: favoriteStops.filter(l => l !== stop.id) }, { merge: true })
    } else {
      setFavoriteStops([...favoriteStops, stop.id])
      await setDoc(userRef, { favoriteStops: [...favoriteStops, stop.id] }, { merge: true })
    }
  }
