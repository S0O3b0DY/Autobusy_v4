...
// src/parser.ts
function shrinkCsv(csvUint8, keepColumns) {
...
__name(splitLine2, "splitLine2");

// src/stops.ts
function buildStopsJson(raw) {
...
__name(buildStopsJson, "buildStopsJson");

// src/timetable.ts
function buildTimetable(files) {
...
__name(buildTimetable, "buildTimetable");

// src/gtfs-processor.ts
var GtfsProcessor = class {
  static {
    __name(this, "GtfsProcessor");
  }
  state;
  env;
  ...
  async process(files, kv) {
    ...
    try {
      ...
      console.log("[GtfsProcessor] Budowanie timetable (674k rekord\xF3w)...");
      ...
      await kv.put("meta:updated_at", (/* @__PURE__ */ new Date()).toISOString(), { expirationTtl: TTL });
...
var src_default = {
  ...
  async fetch(request, env) {
    ...
    if (mStop) {
      try {
        const { data: data2, error: error2 } = await supabase.from("gtfs_stops").select("stop_code").eq("stop_id", mStop[1]);
        if (error2) throw error2;
        if (!data2 || data2.length === 0) return apiError(404, "STOP_NOT_FOUND", "Stop not found in database");
        const upstreamUrl = `http://rozklady.lodz.pl/Home/GetTimetableReal?busStopNum=${data2[0].stop_code}`;
        return await fetchAndParseUpstream(upstreamUrl, parseStopXML, [mStop[1], data2[0].stop_code], supabase);
      } catch (err) {
    ...
    if (path === "/stops") {
      try {
        ...
        let hasMore = true;
        while (hasMore) {
          const { data: data2, error: error2 } = await supabase.from("gtfs_stops").select("stop_id, stop_code, stop_name, stop_lat, stop_lon, zone_id").range(from, to);
          if (error2) throw error2;
          if (data2.length > 0) {
    ...
    if (mRoute) {
      ...
      const formattedDataStops = data.map((item) => ({
        id: item.stop_id,
        c: item.stop_code,
        n: item.stop_name,
        y: item.stop_lat,
    ...
    let mVehicles = path.match(/^\/vehicles\/([^/]+)$/);
    if (mVehicles) {
      const lines = mVehicles[1].split(",");
      if (lines.length > 50) {
        return apiError(400, "Bad Request", "Max 50 lines");
      }
      let sessionCookie = "";
      let vehicleData = [];
      try {
      ...
      async function fetchVehicleData(line, direction = "", nb = "", sessionCookie2) {
        const formData = new URLSearchParams();
        formData.append("r", line);
        formData.append("d", direction);
        formData.append("nb", nb);
        try {
          const rawData = await fetch("http://rozklady.lodz.pl/Home/CNR_GetVehicles", {
            method: "POST",
            headers: {
              ...
              "Cookie": sessionCookie2 || "ASP.NET_SessionId=bh4heyp4q1o43jrt0251elo2"
            },
          ...
          return { line, data: dataXML };
        } catch (error2) {
          console.error(`B\u0142\u0105d pobierania dla linii ${line}:`, error2);
          return { line, data: null, error: error2.message };
        }
      }
      __name(fetchVehicleData, "fetchVehicleData");
      const promises = lines.map((line) => fetchVehicleData(line, "", "", sessionCookie));
      const results = await Promise.all(promises);
      console.log(results);
    }
    let mNextStops = path.match(/^\/vehicles\/([^/]+)\/next-stops$/);
    if (mNextStops) {
    ...
    if (path === "/list") {
      const upstreamUrl = `http://rozklady.lodz.pl/Home/GetRouteList`;
      return await fetchAndParseUpstream(upstreamUrl, parseVehicleList, null, supabase);
    }
    return apiError(404, "NOT_FOUND", "The requested endpoint does not exist");
  }
};
async function fetchAndParseUpstream(url, parserFn, args, supabase) {
  const targetDate = new Date(2019, 10, 27);
  const age = String(Math.floor((Date.now() - targetDate.getTime()) / (1e3 * 60 * 60 * 24)));
  let upstream;
  try {
...
__name(fetchAndParseUpstream, "fetchAndParseUpstream");
async function parseStopXML(xmlText, args, supabase) {
  const timetable = {
    stopID: args[0],
    stopCode: args[1] || 0,
    name: "N/A",
  ...
  await new HTMLRewriter().on("Schedules", {
    element(el) {
  ...
  }).transform(new Response(xmlText, { headers: { "content-type": "text/xml" } })).arrayBuffer();
  return timetable;
}
__name(parseStopXML, "parseStopXML");
async function parseVehicleNextStopsXML(xmlText, args, supabase) {
  const timetable = { routeId: 0, busLine: "N/A", vehType: "N/A", dest: "N/A", dep: [] };
  await new HTMLRewriter().on("Schedules", {
    element(el) {
...
__name(parseVehicleNextStopsXML, "parseVehicleNextStopsXML");
async function parseVehicleList(dataString, args, supabase) {
  const data = JSON.parse(dataString);
  const result = { buses: [], trams: [] };
...
__name(parseVehicleList, "parseVehicleList");
...
// .wrangler/tmp/bundle-1s0Mxd/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-1s0Mxd/middleware-loader.entry.ts
