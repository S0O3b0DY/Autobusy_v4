...
var src_default = {
  ...
  async fetch(request, env) {
    ...
    let mRoutes = path.match(/^\/routes\/([^/]+)$/);
    if (mRoutes) {
      const routeId = mRoutes[1].trim();
    ...
    if (mVehicles) {
      const upstreamUrl = `http://rozklady.lodz.pl/myBusServices/SchedulesService.svc/GetVehicles?cRouteLst=${mVehicles[1]}`;
      return await fetchAndParseUpstream(upstreamUrl, parseVehicleXML, null, supabase);
    }
...
async function parseVehicleXML(xmlText, args, supabase) {
  const vehicles = [];
  await new HTMLRewriter().on("V", {
    element(el) {
      vehicles.push({
...
__name(parseVehicleXML, "parseVehicleXML");
...
// .wrangler/tmp/bundle-GbERFn/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-GbERFn/middleware-loader.entry.ts
