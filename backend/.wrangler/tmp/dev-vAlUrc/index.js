...
var src_default = {
  ...
  async fetch(request, env) {
    ...
    if (m) {
      const { data, error } = await supabase.from("gtfs_stops").select("stop_code").eq("stop_id", m[1]);
      if (error) throw error;
      upstreamUrl = `http://rozklady.lodz.pl/Home/GetTimetableReal?busStopNum=${data[0].stop_code}`;
      parserFn = parseStopXML;
      args = [m[1], data[0].stop_code];
    }
    m = path.match(/^\/routes\/([^/]+)$/);
    if (m) {
      const routeId = m[1];
      ...
      const { data, error } = await supabase.rpc("get_route_shape_by_date", {
        p_route_id: routeId,
        p_date: formattedDate
      });
      if (error) {
        console.error("B\u0142\u0105d pobierania kszta\u0142tu trasy:", error);
      }
      return new Response(JSON.stringify({ routeId, data, error }), {
        status: 200,
    ...
    if (m) {
      ...
      let parsed;
      try {
        parsed = await parseVehicleNextStopsXML(dataXML, args, supabase);
      } catch {
    ...
    if (upstreamUrl) {
      ...
      let data;
      try {
        data = parserFn ? await parserFn(xmlText, args, supabase) : { raw: xmlText };
      } catch {
        return apiError(500, "XML_PARSE_ERROR", "failed to parse upstream xml");
      }
...
// .wrangler/tmp/bundle-OhMjxu/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-OhMjxu/middleware-loader.entry.ts
